...
COLLISION_DIST = 0.3  ####  충돌로 인식할 레이저 스캔 거리의 한계값
TIME_DELTA = 0.2   ### 2
...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.goal_reached = False  # 목표 도달 여부 플래그 추가####
    ...
    def step(self, action):
        ...
        if (distance < GOAL_REACHED_DIST) :  # and (distance != 0)
            env.get_logger().info("GOAL is reached!")
            target = True
            done = True
            self.goal_reached = True  # 목표 도달 시 플래그 설정
    ...
    def reset(self):
        ...
        try:
            ...
            self.get_logger().info('reset : here~~~~~')
        ...
        self.get_logger().info('goal 고르는중 ..')

        self.change_goal()
        # if self.goal_reached:  ####
        #     self.change_goal()  # 목표에 도달했을 때만 목표 변경
        #     self.goal_reached = False  # 플래그 초기화
        self.get_logger().info('goal 고름 ..')
    ...
    def change_goal(self):
        ...
        while not goal_ok:
            ...
            env.get_logger().info(f"goal = {self.goal_x, self.goal_y}")
    ...
    def random_box(self):
        ...
        for i in range(4):
            ...
            box_state.pose.orientation.w = 0.5
    ...
    def observe_collision(laser_data):
        ...
        if (min_laser < COLLISION_DIST) and (min_laser != 0):
...
class Velodyne_subscriber(Node):
    ...
    def has_inf_in_cloud(self, cloud):
        return any(np.isinf(coord) for point in read_points(cloud, skip_nans=True) for coord in point[:3])
...
if __name__ == '__main__':
    ...
    load_model = True  # Weather to load a stored model  False
